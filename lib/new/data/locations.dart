import 'package:new_app/new/data/reviews.dart';
import 'package:new_app/new/model/location.dart';

List<Location> locations = [
  Location(
    name: 'Vivekananda Rock Memorial',
    urlImage:
        'https://www.tamilnadutourism.tn.gov.in/img/pages/large-desktop/vivekananda-rock-memorial-1655195795_ac72566bd8720c988c09.webp',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'NORTH LAT 24',
    longitude: 'EAST LNG 17',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Thiruvalluvar Statue',
    urlImage:
        'https://upload.wikimedia.org/wikipedia/commons/1/1f/Thiruvalluvar_Statue_At_Kanyakumari_%28197523127%29.jpeg',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 11641',
    starRating: 4,
    latitude: 'SOUTH LAT 14',
    longitude: 'EAST LNG 27',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Kanyakumari Temple',
    urlImage:
        'https://www.templedarshanyatri.com/upload/meenakshi-amma-temple-1.png',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'NORTH LAT 24',
    longitude: 'WEST LNG 08',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Gandhi Memorial',
    urlImage:
        'https://kanyakumaritourism.in/images//tourist-places/mahatma-gandhi-memorial-mandapam-kanyakumari/mahatma-gandhi-memorial-mandapam-kanyakumari-tourism-holidays-closed-on-timings.jpg',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'SOUTH LAT 39',
    longitude: 'WEST LNG 41',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Natural Attractions',
    urlImage:
        'https://beachesofindia.in/wp-content/uploads/2017/07/Untitled-design-68-1.jpg',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'SOUTH LAT 39',
    longitude: 'WEST LNG 41',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Museums and Educational Sites',
    urlImage:
        'https://www.hlimg.com/images/things2do/738X538/ttd_1544975701m1.jpg?w=400&dpr=2.6',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'NORTH LAT 24',
    longitude: 'EAST LNG 17',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Wildlife and Nature Reserves',
    urlImage:
        'https://www.keralaholidays.com/uploads/tourpackages/main/backwater.jpg',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 11641',
    starRating: 4,
    latitude: 'SOUTH LAT 14',
    longitude: 'EAST LNG 27',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Swami Vivekanandas House',
    urlImage:
        'https://imgmedia.lbb.in/media/2020/12/5fc5fbe2740d0a4ca16582c0_1606810594841.jpg',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'NORTH LAT 24',
    longitude: 'WEST LNG 08',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Muttom Beach',
    urlImage:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSB4w4Wk9gaXbBHR2k-80kYFVGmnJKxEPMUSA&s',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'SOUTH LAT 39',
    longitude: 'WEST LNG 41',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Mathur Hanging Bridge',
    urlImage:
        'Mathur Hanging Bridgehttps://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS9OIBchoiD1kOlHpW5eCWvDMy36nWvCXjcug&s',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'SOUTH LAT 39',
    longitude: 'WEST LNG 41',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Beautiful waterfalls.',
    urlImage:
        'https://arulshaji.com/wp-content/uploads/2023/09/thirparappu-waterfalls-1024x768.jpg',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'NORTH LAT 24',
    longitude: 'EAST LNG 17',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Art Gallery at Padmanabhapuram Palace ',
    urlImage:
        'https://media1.thrillophilia.com/filestore/fb0zd9aird8yrlb75krijbwsihh8_1580473344_shutterstock_498424846.jpg',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 11641',
    starRating: 4,
    latitude: 'SOUTH LAT 14',
    longitude: 'EAST LNG 27',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Kanyakumari Eco Park ',
    urlImage:
        'https://kanyakumaritourism.in/images/places-to-visit/headers/kanyakumari-eco-park-tourism-entry-fee-timings-holidays-reviews-header.jpg',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'NORTH LAT 24',
    longitude: 'WEST LNG 08',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Azhikal Beach.',
    urlImage:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRv9514RBD050OWblV38HHUFZKFXk4Qpss-ew&s',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'SOUTH LAT 39',
    longitude: 'WEST LNG 41',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Kanyakumari Art Gallery',
    urlImage:
        'https://content.jdmagicbox.com/comp/def_content/art-galleries/l1bqhomyta-art-galleries-7-kej42.jpg',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'SOUTH LAT 39',
    longitude: 'WEST LNG 41',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Vattakottai Fort',
    urlImage:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTZ8iXctoSowXL029d4liOZcOU1I20o1wSr0w&s',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'NORTH LAT 24',
    longitude: 'WEST LNG 08',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Bharat Mata Temple ',
    urlImage: 'https://pbs.twimg.com/media/F_h_zyza0AAis6Z.jpg',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'SOUTH LAT 39',
    longitude: 'WEST LNG 41',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'Kallumalai Murugan Temple',
    urlImage:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/a/aa/Kallumalai_Murugan_Temple%2C_Ipoh%2C_Malaysia.jpg/2560px-Kallumalai_Murugan_Temple%2C_Ipoh%2C_Malaysia.jpg',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'SOUTH LAT 39',
    longitude: 'WEST LNG 41',
    reviews: Reviews.allReviews,
  ),
  Location(
    name: 'mukkadal dam',
    urlImage:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQYNg5xA6GyeVB664WDQo3GtMc8vjEoRSAbsRKLIlZwdv_K5Wxvg8Bo8fwaWcx_8oesFIY&usqp=CAU',
    addressLine1: 'La Cresenta-Montrose, CA91020 Glendale',
    addressLine2: 'NO. 791187',
    starRating: 4,
    latitude: 'SOUTH LAT 39',
    longitude: 'WEST LNG 41',
    reviews: Reviews.allReviews,
  )
];
